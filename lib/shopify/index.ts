...
// Shopify API request with Storefront Access Token
async function shopifyFetch<T>({
  ...
  try {
    ...
    if (SHOPIFY_STOREFRONT_ACCESS_TOKEN) {
      // Private tokens (shpat_*) use a different header than public tokens
      if (SHOPIFY_STOREFRONT_ACCESS_TOKEN.startsWith('shpat_')) {
        headers['Shopify-Storefront-Private-Token'] = SHOPIFY_STOREFRONT_ACCESS_TOKEN
      } else {
      ...
      products(first: $first, sortKey: $sortKey, reverse: $reverse) {
        edges {
          node {
            ...
            availableForSale
            productType
            metafield(namespace: "custom", key: "cursusdatum") {
              value
            }
            options {
              id
...
// Create cart with Dutch buyer identity for BTW calculation
...
// Create direct checkout (bypasses cart, preserves return_to parameter)
